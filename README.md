...
| Per-second salary accrual | **Live** | A registered 1 USDC `SalaryStream` accrues on Testnet; employer and employee may inspect it, while only the immutable employee may withdraw |
...
- `GET /api/streams/:id` — registered employer-or-employee read after exact
  mandate and employee verification against Testnet.
