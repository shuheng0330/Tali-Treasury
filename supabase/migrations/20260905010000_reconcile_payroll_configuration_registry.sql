-- The payroll registry was redesigned after the original 20260904020000
-- migration had already been applied by local and preview environments. Since
-- Supabase identifies migrations by their timestamp, replacing that file does
-- not upgrade those databases. Preserve the unverifiable legacy snapshots for
-- audit/debugging and install the strict immutable registry alongside them.
...
begin
  if exists (
    ...
    where table_schema = 'public'
      and table_name = 'payroll_configurations'
      and column_name = 'setup_digest'
  ) then
    alter table public.payroll_configurations
      rename to payroll_configurations_legacy_20260904;
    ...
    revoke all on public.payroll_configurations
      from public, anon, authenticated, service_role;
    grant select, insert on public.payroll_configurations to service_role;

    revoke all on public.payroll_configurations_legacy_20260904
      from public, anon, authenticated, service_role;
    grant select on public.payroll_configurations_legacy_20260904 to service_role;
  end if;
end;
$$;
