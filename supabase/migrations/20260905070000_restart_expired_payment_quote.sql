-- A failed payment is a confirmed no-transfer outcome. A MYR quote may expire
-- before its retry, so allow exactly this reset into the normal quote and
-- human-review workflow. Paying/paid claims still cannot alter their quote.
...
declare
  ...
  expired_payment_retry_reset boolean := false;
begin
  if tg_op = 'UPDATE' then
    ...
    expired_payment_retry_reset :=
      old.state = 'payment_failed' and new.state = 'submitted' and
      new.fx_quote is null and new.decision is null and
      new.review_action is null and new.reviewer_wallet is null and
      new.review_reason is null and new.reviewed_at is null and
      new.payment is null and new.payment_attempt_digest is null and
      new.payment_attempt_budget_before is null and
      new.payment_attempt_prepared_at is null and
      new.payment_attempt_last_checked_at is null;
  end if;

  if tg_op = 'UPDATE' and old.fx_quote is not null and not correction_reset and not expired_payment_retry_reset and (
  ...
  if q is distinct from (case when tg_op = 'UPDATE' then old.fx_quote else null end) then
    if not correction_reset and not expired_payment_retry_reset and (
