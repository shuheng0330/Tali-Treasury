...
function Segment({ name, amount, share, className }: { name: string; amount: Amount; share: number; className: string }) {
  return (
    <div className="flex min-w-0 flex-col gap-1">
      <span className="text-label uppercase text-ink-3">{name}</span>
    ...
    <section className="flex flex-col gap-4">
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        <div className="flex items-baseline gap-2">
          <span className="tnum text-display">{toDisplay(available)}</span>
      ...
      <div
        ...
        className="relative flex h-6 w-full overflow-hidden rounded-badge bg-sunken"
      >
      ...
      <div className="grid grid-cols-3 gap-4">
        <Segment name="Settled" amount={settled} share={settledPct} className="bg-accent" />
        <Segment name="Committed" amount={committed} share={committedPct} className="bg-accent/45" />
