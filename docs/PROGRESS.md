...
| Authenticated Set Up Payroll | 🟡 Complete locally through verified wallet execution | Employer-only screen, live FX conversion, RM30/RM50 demo limits, wallet funding, finality check and strict mandate/cap verification | Add idempotent durable registration and recovery |
...
3. Finish authenticated **Set Up Payroll** registration. Wallet preview, signing
   and server-side verification are complete locally; persist the verified result
   idempotently without a second funding transaction.
