...
| Create Expense Treasury | 🟡 Registration slice complete locally | `/treasury/setup`: treasurer form, USDC funding preview, wallet-signed creation against the published package, finalized mandate/cap verification, idempotent `POST /api/events`, recipient membership seeding, and in-page digest recovery after a remount that never funds again | Event selection/routing and event-aware capability mapping; Testnet smoke retry |
...
6. Preserve the working expense-claim demo. Protected registration is now behind
   the separate **Create Expense Treasury** screen; add event selection/routing
   and event-aware capability mapping.
